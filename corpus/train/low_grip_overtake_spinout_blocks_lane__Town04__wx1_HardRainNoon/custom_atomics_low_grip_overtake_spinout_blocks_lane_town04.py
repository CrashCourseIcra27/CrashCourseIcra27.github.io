...
class StopActorsOnEgoCollision(py_trees.behaviour.Behaviour):
    ...
    def __init__(
        ...
        freeze_actors_on_collision: bool = False,
        disable_other_collision: bool = False,
        stop_ego_on_collision: bool = True,
        stabilize_ego_on_collision: bool = False,
    ):
        super().__init__(name)
        ...
        self._freeze_actors_on_collision = freeze_actors_on_collision
        self._disable_other_collision = disable_other_collision
        self._stop_ego_on_collision = stop_ego_on_collision
        self._stabilize_ego_on_collision = stabilize_ego_on_collision
    ...
    def _on_collision(self, event):
        ...
        is_walker = (other_actor is not None and
                      getattr(other_actor, "type_id", "").startswith("walker."))
        if self._disable_other_collision and is_walker:
            try:
                other_actor.set_collision_detection(False)
            except (AttributeError, RuntimeError):
                pass
        if self._stabilize_ego_on_collision and is_walker:
            try:
                velocity = self._ego.get_velocity()
                speed = max(12.5, (velocity.x ** 2 + velocity.y ** 2) ** 0.5)
                forward = self._ego.get_transform().get_forward_vector()
                self._ego.set_target_angular_velocity(carla.Vector3D(0.0, 0.0, 0.0))
                self._ego.set_target_velocity(carla.Vector3D(
                    forward.x * speed, forward.y * speed, 0.0))
            except (AttributeError, RuntimeError):
                pass
        if not self._freeze_actors_on_collision:
            return
        for actor in self._actors:
            if (
                actor is not None
                and actor.is_alive
                and other_actor is not None
                and actor.id == other_actor.id
            ):
                try:
    ...
    def update(self):
        if not self._impact_detected:
            return py_trees.common.Status.RUNNING
        if not self._stop_ego_on_collision:
